...
- Open [http://localhost:8090/actuator/health](http://localhost:8080/actuator/health) and you should get `{"status":"UP"}`

> **Note:** The application runs on port `8090` by default to avoid conflicts with other apps in the infrastructure (for example, PowerAuth).
...
--data '{
    ...
    "certificate_request": "MIICfDCCAWQCAQ...",
...
> **Note:** Remove `-----BEGIN CERTIFICATE REQUEST-----` and `-----END CERTIFICATE REQUEST-----` from the `certificate_request` value. \
