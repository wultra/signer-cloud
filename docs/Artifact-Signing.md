...
All Signer Cloud Docker images are signed using [Cosign](https://github.com/sigstore/cosign). Each image also has an attached Software Bill of Materials (SBOM) in [CycloneDX](https://cyclonedx.org/) format, expect the init image.
...
cosign verify \
    --key cosign.pub \
    powerauth/signer-cloud-server:${VERSION}
```
